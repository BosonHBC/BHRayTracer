...
bool Box::IntersectRay(Ray const &r, float t_max) const
{
	...
	Vec3f v0 = Vec3f(-1, -1, 0); // in XY plane1 and ZX plane1 and YZ plane1
	Vec3f v7 = Vec3f(1, 1, 1); // in  XY plane2 and ZX plane2 and YZ plane2
	...
	if (tMin <= tMax && tMin < t_max && tMin > 0) return true;
	return false;
}
